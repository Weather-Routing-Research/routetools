...
def control_to_curve(
    control: np.ndarray, src: np.ndarray, dst: np.ndarray, L: int = 64
) -> np.ndarray:
    """
...
def optimize(
    ...
    **kwargs: optional additional parameters for CMA-ES
    ...
    while not es.stop():
        ...
        curve = control_to_curve(np.array(X), src, dst, L=L)
        cost = cost_function(u, v, curve, travel_speed, travel_time, L=L)
        es.tell(X, cost)
        if verbose:
            es.disp()
    if verbose:
